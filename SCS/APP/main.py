...
#ABILITA OPTO
stream = os.popen('sudo echo 354 > /sys/class/gpio/export')
output = stream.read()
stream = os.popen('sudo echo out > /sys/class/gpio/gpio354/direction')
output = stream.read()
stream = os.popen('sudo echo 1 > /sys/class/gpio/gpio354/value')
output = stream.read()
...
ser = SerialHandler(
		port='/dev/ttyS3', #Replace ttyS0 with ttyAM0 for Pi1,Pi2,Pi0
		baudrate = 9600
)
